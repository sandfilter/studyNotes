...
var _CAL = {
	...
	getDateDay: function(dateStr){
		...
		var dateFormate = new Date(arr[0], arr[1], arr[2]);
		return dateFormate.getTime() / (24 * 60 * 60 * 1000);
	},
	/**
	 * 将天数转变为yyyy-mm-dd格式的日期字符串
	 * @param  {[type]} daysNum [description]
	 * @return {[type]}         [description]
	 */
	getDateFormate: function(daysNum){
		var dateFormate = new Date(daysNum * 24 * 60 * 60 * 1000);
		var strFormate = dateFormate.getFullYear() + "-" + _CAL.formatNum(dateFormate.getMonth()) + "-" + _CAL.formatNum(dateFormate.getDate());
		return strFormate;
	},
	/**
...
Calendar.prototype = {
	...
	_init: function(config) {
		 ...
		 * ajax函数
		 */
		this.funAjax = config.funAjax || function() {};
		/**
		 * 选择日期回调方法
	...
	_create: function() {
		...
		//显示上月按钮
		this.isPrevBtn  && aTmp.push("<span class=\"cal-prev\"><<</span>");
		//显示下月按钮
		this.isNextBtn  && aTmp.push("<span class=\"cal-next\">>></span>");
			...
			if(oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {
				...
				if(that.triggerNode.tagName.toUpperCase() === "INPUT") {
					if(lzc.id(that.id).className == 'time-end'){
						...
						if(lzc.id(that.id).className == 'cal-end'){
							var startDay = _CAL.getDateDay(lzc.tagName(that.triggerNode.tagName, that.triggerNode.parentNode)[0].value), curDay = _CAL.getDateDay(oTarget["data-date"]), dateInfo = '';
							dateInfo = obj["data-date"];
							if(that.isDateInfo) that.setDateInfo(dateInfo);
							that.funAjax();
						}else{
	...
	setDateInfo: function(sDate) {
		...
		if(lzc.id(this.id).className != 'time-end'){
			(this.triggerNode.value = sDate, this.render(sDate), this.getDateInfo(sDate));
		}else{
			this.render(sDate);
		}
	},
